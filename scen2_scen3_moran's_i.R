...
s2_m_fdr_plot <- ggplot(s2_morans_fdr, aes(phi, morans_i_fdr)) +
    geom_line() +
    scale_y_continuous(limits = c(0, 0.5), breaks = c(0, 0.05, 0.1, 0.3, 0.5)) +
...
one_run_rho_moran <- function(n = nsims_less, rho) {
    out <- data.frame()
    for (u in 1:n) {
        x1 <- sample(x = c(0, 1), size = N, replace = TRUE)
...
for (r in rho_range) {
    ...
    s3_moransi <- rbind(s3_moransi, one_run_rho_moran(rho = r))
}

# Moran's I false discovery rate
fdr_fun <- function(x) (sum(x < 0.05)/length(x))

s3_morans_fdr <- s3_moransi %>% group_by(rho) %>%
    summarise(morans_i_fdr = fdr_fun(morans_i_p_value))

save(s2_morans_fdr, s3_morans_fdr, file = 'mc_results/morans_i_fdr.rda')

s3_m_fdr_plot <- ggplot(s3_morans_fdr, aes(rho, morans_i_fdr)) +
    geom_line() +
    scale_y_continuous(limits = c(0, 0.5), breaks = c(0, 0.05, 0.1, 0.3, 0.5)) +
    geom_hline(yintercept = 0.05, linetype = 'dotted') +
    ylab('FDR\n') + xlab(expression(rho)) +
