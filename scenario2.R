...
ps_df <- extract_element(s2_under_list, 'pvalue', 'lag_wy')

s2_p_under <- ggplot(ps_df, aes(value)) +
    geom_density() +
...
ps_df <- extract_element(s2_over_list, 'pvalue', 'lag_wy')

s2_p_over <- ggplot(ps_df, aes(value)) +
    geom_density() +
    scale_x_continuous(breaks = c(0, 0.05, 0.1, 0.2, 0.5, 1), limits = c(0, 1)) +
    geom_vline(xintercept = 0.05, linetype = 'dashed') +
    xlab('\np-value of temporally-lagged spatial lag') +
